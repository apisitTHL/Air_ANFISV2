...
def print_membership_functions(mynet,log_widget):
    ...
    for i in range(ni):
        ...
        log_widget.insert('end', f"Input {i + 1} Membership Functions:\n")
        # print(f"Input {i+1} Membership Functions:")
        for j in range(mf):
            params = mparams[i * mf + j]
            a, b, c = params
            # print(f"  MF{j+1}: gbellmf(x, [{a}, {b}, {c}])")
            log_widget.insert('end', f"  MF{j + 1}: gbellmf(x, [{a}, {b}, {c}])\n")

    log_widget.insert('end', "\n")
    log_widget.insert('end', "MF RULE OUTPUT")
    log_widget.insert('end', f"  {kparam_val}])\n")
